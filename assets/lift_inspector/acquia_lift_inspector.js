    ...
    attach: function (context, settings) {
      ...
      function show_inspector_modal() {
        ...
        modal_template = _.template(
          ...
          '     <div><label>' + Drupal.t('Customer') + '</label><p><a href="' + accountURL + '#person:accountId=<%- account_id %>" target="_blank" rel="noopener noreferrer"><%- account_id %></a></p></div>' +
          '     <div><label>' + Drupal.t('Customer Site') + '</label><pre><%- site_id %></a></pre></div>' +
          '     <div><label>' + Drupal.t('Tracking ID') + '</label><pre><a href="' + accountURL + '#person:accountId=<%- account_id %>;tracking=<%- lastDecisionRequest.identity %>" target="_blank" rel="noopener noreferrer"><%- lastDecisionRequest.identity %></a></pre></div>' +
          '    </div>' +
          '  </div>' +
          ...
          '           <div><label>' + Drupal.t('Description') + '</label><p><%- segment.description %></p></div>' +
          '         </div>' +
          '        <% }); %>' +
          '      <% } else { %>' +
          ...
          '            <div><label>' + Drupal.t('Slot Name') + '</label><p><%- decision.slot_name %></p></div>' +
          '            <div><label>' + Drupal.t('Slot View Mode') + '</label><p><%- decision.view_mode_id %></p></div>' +
          '            <div><label>' + Drupal.t('Rule Name') + '</label><p><%- decision.rule_name %></p></div>' +
          '            <div><label>' + Drupal.t('Content Displayed') + '</label><p><%- decision.content_name %></p></div>' +
          '          </div>' +
          '        <% }); %>' +
          '      <% } else { %>' +
          ...
          '          <div><label>' + Drupal.t('Name') + '</label><p><%- capture.event_name %></p></div>' +
          '          <div><label>' + Drupal.t('Source') + '</label><p><%- capture.event_source %></p></div>' +
          '          <div class="inline-wrap"><label class="inline-pre">' + Drupal.t('URL') + '</label><pre><%- capture.url %></pre></div>' +
          '          <div> <label>' + Drupal.t('Date') + '</label><p><%- capture.event_date %></p></div>' +
          '          </div>' +
          '        <% }); %>' +
          '      <% } else { %>' +
